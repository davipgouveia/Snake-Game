  ...
  // Utilitário A* para grid
  function findPathAStar(start, goal, grid, forbidden) {
    "use client";
    // Auto-Play Bot
      const [autoPlay, setAutoPlay] = useState(false);
      const autoPlayRef = useRef(false);
      ...
      function findPathAStar(start, goal, grid, forbidden) {
        ...
        while (queue.length) {
          // Ordena por heurística (distância de Manhattan)
          queue.sort((a, b) =>
            (a.path.length + Math.abs(a.pos.x - goal.x) + Math.abs(a.pos.y - goal.y)) -
            (b.path.length + Math.abs(b.pos.x - goal.x) + Math.abs(b.pos.y - goal.y))
          );
          ...
          for (const d of Object.values(DIRECTIONS)) {
            ...
            if (
              ...
              forbidden.some((f) => f.x === next.x && f.y === next.y)
            ) /* Line 26 omitted */
      ...
      // Bot: calcula direção ótima e move
      useEffect(() => {
        if (!autoPlay) return;
        ...
        const id = setInterval(() => {
          ...
          const path = findPathAStar(head, foodRef.current, boardSize, forbidden);
          if (path && path.length) {/* Lines 46-54 omitted */}
        }, Math.max(40, difficulty.speed));
        return () => clearInterval(id);
      }, [autoPlay, status, boardSize, difficulty.speed]);
    return () => clearInterval(id);
  }, [autoPlay, status, boardSize, difficulty.speed]);
